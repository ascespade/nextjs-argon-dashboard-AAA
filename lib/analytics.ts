...
export class AnalyticsManager {
    ...
    // Track user action
    trackUserAction(action: string, componentId?: string, componentType?: string) {
    ...
    // Track editor action
    trackEditorAction(action: string, componentId?: string, componentType?: string) {
    ...
    getPageAnalytics(pageId: string): PageAnalytics | null {
        const pageEvents = this.events.filter(e =>
            e.metadata?.pageId === pageId || e.pageUrl.includes(pageId)
        );

        if (pageEvents.length === 0) return null;

        const views = pageEvents.filter(e => e.category === 'page' && e.action === 'view').length;
        const uniqueViews = new Set(pageEvents.map(e => e.userId || e.sessionId)).size;

        const timeOnPageEvents = pageEvents.filter(e => e.category === 'performance' && e.action === 'timeOnPage');
        const avgTimeOnPage = timeOnPageEvents.length > 0
            ? timeOnPageEvents.reduce((sum, e) => sum + (e.value || 0), 0) / timeOnPageEvents.length
            : 0;

        const bounceEvents = pageEvents.filter(e => e.category === 'page' && e.action === 'bounce');
        const bounceRate = views > 0 ? (bounceEvents.length / views) * 100 : 0;

        const lastViewed = pageEvents.length > 0

...
export class SEOTools {
    ...
    static generateMetaTags(seoData: SEOData): string {
        ...
        if (seoData.keywords && seoData.keywords.length > 0) {
            tags.push(`<meta name="keywords" content="${this.escapeHtml(seoData.keywords.join(', '))}">`);
        }
    ...
    static generateSitemapEntry(url: string, lastmod?: string, changefreq?: string, priority?: number): string {
  ...
  <url>
    ...
    static generateRobotsTxt(allowAll: boolean = true, disallowPaths: string[] = [], sitemapUrl?: string): string {
        const lines: string[] = [];
